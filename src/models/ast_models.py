...
import timm
# from timm.models.layers import to_2tuple,trunc_normal_,patch_embed
...
class ASTModel(nn.Module):
    ...
    def __init__(self, label_dim=527, fstride=10, tstride=10, input_fdim=128, input_tdim=1024, imagenet_pretrain=True, audioset_pretrain=False, model_size='224',patch_size=16, verbose=True):
        ...
        if audioset_pretrain == False:
            self.origin = False # origin means we use DeiT-384 as backbone, just an ablation. Nomally we use ViT-224 as backbone
            if model_size=='origin':
                self.origin = True
                self.v = timm.create_model('deit_base_distilled_patch16_384', pretrained=imagenet_pretrain,embed_layer=PatchEmbed)
            elif patch_size == 8:
                self.v = timm.create_model('vit_base_patch8_224', pretrained=imagenet_pretrain,embed_layer=PatchEmbed)
            elif patch_size == 16:
                self.v = timm.create_model('vit_base_patch16_224', pretrained=imagenet_pretrain,embed_layer=PatchEmbed)
            elif patch_size == 32:
                self.v = timm.create_model('vit_base_patch32_224', pretrained=imagenet_pretrain,embed_layer=PatchEmbed)
            ...
            f_dim, t_dim = self.get_shape(fstride, tstride, patch_size,input_fdim, input_tdim)
            ...
            if imagenet_pretrain == True:
                # sum 3 channel of ViT weights to 1 channel for spectrogram
            ...
            if imagenet_pretrain == True:
                ...
                # for the ViT only need to skip the first one token
                if model_size=='origin':
    ...
    def get_shape(self, fstride, tstride, patch_size, input_fdim=128, input_tdim=1024):

...
def train(audio_model, train_loader, test_loader, args):
    ...
    if args.resume:
        # /mnt/work2/users/fj/FlexiAST/distill/voxceleb/PI_T8_Spatch16_Base224_-b12-m0/models/latest_audio_model8.pth
        out_dict = {}
        check_dict = torch.load(args.checkpoint)
        for k, v in check_dict.items(): # Adjust the name of dict
            out_dict[k[7:]] = v
        audio_model.load_state_dict(out_dict)
        epoch = int(args.checkpoint.split(".")[-2][-1])
    ...
    if args.resume:
        optimizer.load_state_dict(torch.load(args.o_checkpoint))
    ...
    if args.resume:
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(args.lrscheduler_start, 1000, args.lrscheduler_step)),gamma=args.lrscheduler_decay, last_epoch=0)
    else:
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(args.lrscheduler_start, 1000, args.lrscheduler_step)),gamma=args.lrscheduler_decay)
    ...
    while epoch < args.n_epochs + 1:
        ...
        with tqdm(train_loader) as tepoch:
            for i, (audio_input, labels,path) in enumerate(tepoch):
        ...
        if args.wandb:
            wandb.log({"T_loss":loss_meter.avg,"V_loss":valid_loss, "mAP":mAP,"ACC":acc})
    ...
    with torch.no_grad():
        for i, (audio_input, labels,path) in enumerate(val_loader):
            ...
            if args.model == 'ast':
                ...
                audio_output = audio_model(audio_input, None)
...
def validate_ensemble(args, epoch):
    """
    Like a kind of moving average perdiction
    """
    exp_dir = args.exp_dir
